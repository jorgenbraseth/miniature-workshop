...
class StorageService {
  ...
  async initialize(): Promise<void> {
    await Promise.all([this.initIndexedDB(), this.initOPFS()])
  }
  ...
  private migrateUnit(unit: any): Unit {
    return {
      ...
      lastSyncAt: unit.lastSyncAt
        ? unit.lastSyncAt instanceof Date
          ? unit.lastSyncAt
          : new Date(unit.lastSyncAt)
        : undefined,
  ...
  async getStorageStats(): Promise<StorageStats> {
    ...
    const [units, photos] = await Promise.all([this.getAllUnits(), this.getAllPhotos()])
